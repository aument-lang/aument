...
// integrate with Aument.
//
// In addition to the changes listed below, the following modifications
// were added:
//
//  * Use au_data_malloc and au_data_free in place of malloc and free
//
//  * All functions with the prefix _Py_dg are now prefixed with au_dconv
...
#define MALLOC au_data_malloc
#define FREE au_data_free
...
/* Memory management: memory is allocated from, and returned to, Kmax+1
   pools of memory, where pool k (0 <= k <= Kmax) is for Bigints b with
   b->maxwds == 1 << k.  These pools are maintained as linked lists, with
   freelist[k] pointing to the head of the list for pool k.

   On allocation, if there's no free slot in the appropriate pool, MALLOC
   is called to get more memory.  This memory is not returned to the system
   until Python quits.  There's also a private memory pool that's allocated
   from in preference to using MALLOC.

   For Bigints with more than (1 << Kmax) digits (which implies at least
   1233 decimal digits), memory is directly allocated using MALLOC, and
   freed using FREE.
...
static Bigint *Balloc(int k) {
    ...
    if (k <= Kmax && (rv = freelist[k]))
        ...
        len = (sizeof(Bigint) + (x - 1) * sizeof(ULong) + sizeof(double) -
              ...
              sizeof(double);
        if (k <= Kmax &&
            pmem_next - private_mem + len <= (ssize_t)PRIVATE_mem) {
            rv = (Bigint *)pmem_next;
            pmem_next += len;
        } else {
...
/* multiply the Bigint b by 5**k.  Returns a pointer to the result, or NULL
...
/* Return a 'standard' NaN value.
   ...
   There are exactly two quiet NaNs that don't arise by 'quieting'
   signaling NaNs (see IEEE 754-2008, section 6.2.1).  If sign == 0, return
   the one whose sign bit is cleared.  Otherwise, return the one whose sign
   bit is set.
*/

double au_dconv_stdnan(int sign) {
    U rv;
    word0(&rv) = NAN_WORD0;
    word1(&rv) = NAN_WORD1;
    if (sign)
...
double au_dconv_infinity(int sign) {
    U rv;
...
double au_dconv_strtod(const char *s00, char **se) {
...
/* Additional notes (METD): (1) returns NULL on failure.  (2) to avoid
   memory leakage, a successful call to _Py_dg_dtoa should always be
   matched by a call to _Py_dg_freedtoa. */

char *au_dconv_dtoa(double dd, int mode, int ndigits, int *decpt,
                    int *sign, char **rve) {
...
bump_up:
                    ...
                    /* Strip trailing zeros. This branch was missing from
                       the original dtoa.c, leading to surplus trailing
                       zeros in
