...
static void rehash_table(struct au_bc_vars *vars) {
    ...
    struct au_bc_bucket *new_buckets = calloc(new_len, sizeof(struct au_bc_bucket));
    for(int i = 0; i < vars->buckets_len; i++) {
        ...
        for(int i = 0; i < old_bucket->len; i++) {
            ...
            if(bucket->cap == 0) {
                ...
                bucket->data = realloc(bucket->data, bucket->cap * 2 * sizeof(struct au_bc_var_el));
                bucket->cap *= 2;
            }
...
struct au_bc_var_value *au_bc_vars_add(struct au_bc_vars *vars,
    ...
    const hash_t hash = au_hash(key, len);
