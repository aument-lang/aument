...
#define DISPATCH_JMP do{ \
    ...
    goto *cb[op]; \
        ...
        CASE(OP_MOV_U16): {
            ...
            const uint16_t *ptr = (uint16_t*)(&frame.bc[frame.pc+2]);
...
do { au_value_deref(dest); \
    dest = src; \
    au_value_ref(dest); } while(0)
        CASE(OP_MOV_REG_LOCAL): {
            const uint8_t reg = frame.bc[frame.pc+1];
        ...
        CASE(OP_LOAD_CONST): {
            ...
            if(au_value_get_type(tl->const_cache[c]) != VALUE_NONE) {
                ...
                const struct au_program_data_val *data_val = &p_data->data_val.data[c];
...
#define BIN_OP_ASG(FUN) { \
    ...
    frame.locals[local] = au_value_ ## FUN (frame.locals[local], frame.regs[reg]); \
    DISPATCH; \
}
...
#undef BIN_OP_ASG
        ...
        CASE(OP_RET_LOCAL): {
            ...
            // Move ownership of value in ret_local -> return reg in prev. frame
        ...
        CASE(OP_RET): {
            const uint8_t ret_reg = frame.bc[frame.pc+1];
            // Move ownership of value in ret_reg -> return reg in prev. frame
        ...
        CASE(OP_IMPORT): {
            const uint16_t idx = *((uint16_t*)(&frame.bc[frame.pc+2]));
